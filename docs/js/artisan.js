    ...
    app.factory('Range', ['$filter', function($filter) {
        ...
        var formats = {
            long: {
                ...
                SEMESTER: 'Semester from {from|date:MM/dd/yyyy} to {to|date:MM/dd/yyyy}',
                TRIMESTER: 'Trimester from {from|date:MM/dd/yyyy} to {to|date:MM/dd/yyyy}',
                QUARTER: 'Quarter {from|date:MM/dd/yyyy} {to|date:MM/dd/yyyy}',
                MONTH: '{from|date:MMMM yyyy}',
                WEEK: 'Week {from|isoWeek:1}',
                DAY: '{from|date:EEEE MM/dd/yyyy}',
            },
            short: {
                YEAR: '{from|date:yyyy}',
                SEMESTER: '{from|date:MM/yyyy}-{to|date:MM/yyyy}',
                TRIMESTER: '{from|date:MM/yyyy}-{to|date:MM/yyyy}',
                QUARTER: '{from|date:MM/yyyy}-{to|date:MM/yyyy}',
                MONTH: '{from|date:MMMM}',
                WEEK: 'W{from|isoWeek:1}',
                DAY: '{from|date:EEEE}',
            }
        };
        ...
        function setYearPeriod(date, diff) {
            diff = diff || 0;
            date = date || new Date();
            date = new Date(date.setMonth((date.getMonth() + 1) * diff));
            var range = this;
            range.type = RangeTypes.YEAR;
            var yyyy = date.getFullYear();
            range.from = new Date(yyyy, 0, 1);
            range.to = new Date(yyyy, m, 0, 23, 59, 59, 999);
            return range;
        }
        ...
        function toString() {
            return '[' + $filter('date')(this.from, 'MM-dd-yyyy') + ', ' + $filter('date')(this.to, 'MM-dd-yyyy') + '] \'' + this.fullName() + '\'';
        }
